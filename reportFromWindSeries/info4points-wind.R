...
#		obsagg[is.na(obsagg$ghi),"ghi"]<-999
	...
	if (type =="tmy"){
		tmyheader<-read.csv(paste("tmp/",file,sep=""),sep=";",skip=11,header=FALSE,nrows=1,colClasses=rep("character",1,11))
		tmy<-read.csv(paste("tmp/",file,sep=""),sep=";",skip=13,header=FALSE)
...
#FOR on year hourly data print
	...
	if (type == "serie" | type == "reanalysis" | type == "nc"){# get last 10 years
...
while (n < nlines){
	...
	line=as.character(files$filenames[nline])
		...
		if (typed == "obs" || vard =="GHI_real" || vard =="DNI_real" || vard =="GHI" || vard =="DNI" || vard == "M"){
...
meltall$value=ifelse(!is.na(meltall$value) & meltall$value>100,NA,meltall$value)
...
serieall$value=ifelse(!is.na(serieall$value) & serieall$value>100,NA,serieall$value)
...
print("serieall")
print(summary(serieall))
print("meltall")
print(summary(meltall))
monthserie=aggregate(value~Year+font,data=serieall,FUN=mean,na.rm=TRUE)
...
print(paste("mergedmelted"))
print(summary(mergedmelted))
print(paste("serieall"))
print(summary(serieall))
}
print(paste("meltall"))
print(summary(meltall))
...
#some days
varratio=1*length(unique(selectedyearsreduced))/(length(files[,1]))

somedays<-ggplot(serieall[meltall$jday %in% c(1:14),]) + geom_line(aes(hour,value,color=jday,group=jday)) +theme(legend.position="right",axis.text.x=element_text(angle=90, hjust=0,family="mono")) + ggtitle(paste("Series comparison for site ",tag," and var ",var))+facet_wrap(variable~month)
...
#HEATMAP SERIE
varratio=10*length(unique(serieall$Year))/(length(files[,1]))
summary(serieall)
heatmap<-ggplot(serieall)+geom_raster(aes(jday,hour,fill=value)) + scale_fill_gradientn(colours = c("lightblue","blue","green","yellow","orange","red","purple"),breaks=c(3,5,7,10,12,14,16),limits=c(0,16))+theme(legend.position="right",axis.text.x=element_text(angle=90, hjust=0,family="mono")) + coord_fixed(ratio = varratio) +ggtitle(paste("Series comparison for site ",tag,tag2," and var ",var))+facet_grid(font~Year)
print(paste0("SERIEALL"))
head(serieall)
summary(serieall)
heatmapYears<-ggplot(serieall[serieall$Year == 2015,])+geom_raster(aes(jday,hour,fill=value)) + scale_fill_gradientn(colours = c("lightblue","blue","green","yellow","orange","red","purple"),breaks=c(3,5,7,10,12,14,16),limits=c(0,16))+theme(legend.position="right",axis.text.x=element_text(angle=90, hjust=0,family="mono")) + coord_fixed(ratio = varratio/5) +ggtitle(paste("Series comparison for site ",tag,tag2," and var ",var))+facet_grid(font~Year)
#OLD SCHOOL
...
varratio=length(files[,1])/5
plotc<-ggplot(meltall)+ geom_raster(aes(month,hour,fill=value)) + scale_fill_gradientn(colours = c("lightblue","blue","green","yellow","orange","red","purple"),na.value="transparent",breaks=c(3,5,7,10,12,14,16),limits=c(0,16))+ theme(legend.position="right", axis.text.x=element_text(angle=90, hjust=0,family="mono")) + coord_fixed(ratio = varratio) +ggtitle(paste("Hourly aggregateds by Month:",var,tag,tag2))+facet_grid(metric~font)+scale_x_discrete(limits=c(rep(1:12,each =1)))
#metrics line
#OLD SCHOOL:
...
varratio=12
plotdailyprofile<-ggplot(dailyprofile,aes(hour,value,group=font,color=font))+geom_line()+theme(legend.position="right", axis.text.x=element_text(angle=90, hjust=0,family="mono")) + coord_fixed(ratio = varratio) +ggtitle(paste("Daily profile ",var,tag,tag2))
